...
class UserTrustManager(object):
    ...
    def __init__(self):
        ...
        self._skp = smaug_keystone_plugin.SmaugKeystonePlugin()
    ...
    def add_operation(self, context, operation_id):
        ...
        trust_id = self._skp.create_trust_to_smaug(context)
        try:
            lsession = self._skp.create_trust_session(trust_id)
        except Exception:
            self._skp.delete_trust_to_smaug(trust_id)
            raise
    ...
    def delete_operation(self, context, operation_id):
        ...
        if len(operation_ids) == 0:
            self._skp.delete_trust_to_smaug(auth_info['trust_id'])
